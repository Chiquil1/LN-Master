...
describe('useTtsSession', () => {
  ...
  it('appends prepared chapters without restarting native playback', async () => {
    const { result } = renderHook(useTtsSession);
    const session = await getNativeSession();
    const paragraphs = [{ id: 'chapter-2:0', text: 'Next chapter' }];
    const metadata = { novelName: 'Novel', chapterName: 'Chapter 2' };
    const settings = { rate: 1, pitch: 1 };

    await act(async () => {
      await result.current.appendToQueue(paragraphs, metadata, settings);
    });
