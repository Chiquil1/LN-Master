...
internal object TtsPlaybackStore {
    ...
    fun load(
        ...
        val readableParagraphs = nextParagraphs.filter { it.text.isNotBlank() }
        require(readableParagraphs.isNotEmpty()) {
            "The TTS queue contains no readable paragraphs."
        }

        if (initialIndex < 0) {
            appendQueue(readableParagraphs, nextMetadata)
            return
        }
    ...
    private fun appendQueue(
        nextParagraphs: List<TtsParagraph>,
        nextMetadata: TtsMetadata,
    ) {
        val existingIds = paragraphs.mapTo(mutableSetOf()) { it.id }
        val uniqueParagraphs = nextParagraphs.filter { existingIds.add(it.id) }
        if (uniqueParagraphs.isEmpty()) {
            return
        }

        val previousSize = paragraphs.size
        paragraphs = paragraphs + uniqueParagraphs
        if (metadata == null) {
            metadata = nextMetadata
        }
        ...
        if (state == TtsPlaybackState.COMPLETED) {
            ...
            emitProgress()
            emitState()
            checkNotNull(applicationContext).let(TtsPlaybackService::start)
            speakCurrent()
        } else {
            // Refreshes the queue total without interrupting the utterance.
            emitProgress()
        }
    }
    ...
    private fun speakCurrent() {
        ...
        updateChapterMetadata(paragraph)
        generation += 1
    ...
    private fun updateChapterMetadata(paragraph: TtsParagraph) {
        val currentMetadata = metadata ?: return
        val parts = paragraph.id.split(":", limit = 5)
        if (parts[0] != CHAPTER_PARAGRAPH_PREFIX || parts.size !in 4..5) {
            return
        }
        val encodedChapterName = if (parts.size == 5) parts[4] else parts[3]

        val chapterName = try {
            URLDecoder.decode(encodedChapterName, Charsets.UTF_8.name())
        } catch (_: IllegalArgumentException) {
            return
        }

        if (chapterName.isNotBlank() && chapterName != currentMetadata.chapterName) {
            metadata = currentMetadata.copy(chapterName = chapterName)
        }
    }
    ...
    private const val CHAPTER_PARAGRAPH_PREFIX = "lnreader-chapter"
}
